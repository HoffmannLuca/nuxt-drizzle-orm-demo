...
<template>
  <div class="flex flex-col gap-3">
    ...
    <div class="flex">
      ...
      <div v-if="projects.length" class="flex flex-col gap-4 w-2/3">
        <button class="bg-red-500 p-2 rounded w-fit mx-auto" v-for="project in projects" @click="deleteProject(project.id)">delete - {{ project.id }}</button>
      </div>
    </div>
  </div>
</template>
